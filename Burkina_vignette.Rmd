...
The ```decisionSupport()``` function is part of the package ```decisionSupport``` (Luedeling and Göhring, 2017) in the R programming environment (R Core Team 2017). This package was used for a Monte-Carlo-based selection. 
...
```{r, eval=FALSE}
#setwd([folder, where the input table ‘Example_input_table.csv’ file is saved])
decisionSupport(inputFilePath = "Example_input_table.csv", #input file with estimates
                outputPath = file.path(getwd(),"MCResults"), #output folder
                write_table = TRUE,
                welfareFunction = Example_calc,
...
1.	```welfareFunction``` – the decision model function, in this case ```Example_calc```.
...
Additional clarity on the expected net benefits of Exampleation interventions could be gained by collecting information on these important variables. 
...
Chitata, T., Mugabe, F.T., Kashaigili, J.J., 2014. Estimation of small reservoir Exampleation in semi-arid southern Zimbabwe. Journal of Water Resource and Protection 6, 1017–1028.
...
Kondolf, G.M., Gao, Y., Annandale, G.W., Morris, G.L., Jiang, E., Zhang, J. et al., 2014. Sustainable Example management in reservoirs and regulated rivers: Experiences from five continents. Earth’s Future 2, 256–280.
...
Palmieri, A., Shah, F., Dinar, A., 2001. Economics of reservoir Exampleation and sustainable management of dams. Journal of Environnmental Management 61, 149–163.
...
Schmengler, A.C., 2011. Modeling soil erosion and reservoir Exampleation at hillslope and catchment scale in semi-arid Burkina Faso. PhD thesis. University of Bonn, Bonn, Germany.
